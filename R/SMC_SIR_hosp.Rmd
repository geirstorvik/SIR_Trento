...
x = read.csv("antall_per_dag_hele_landet.csv",header=T,sep=";")
colnames(x)<-c("date","npos","hosp_prev")
x=x[,1:2]
df = data.frame(date=as.Date(x$date),npos=x$npos)
...
cat(sprintf("\r%d", s))
 ...
 w = dbinom(df2$npos[s],Inew_weekly,q)
 #print("w")
 #print(range(w))
